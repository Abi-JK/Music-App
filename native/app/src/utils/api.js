// API utilities for React Native app
const API_BASE = 'https://soundaura.netlify.app';
...
export async function searchSongs(query, limit = 40) {
  const url = `${API_BASE}/saavn-search/search/songs?query=${encodeURIComponent(query)}&limit=${limit}`;
...
export async function getStreamUrl(songId) {
  try {
    const data = await safeFetch(`${API_BASE}/saavn-search/songs?id=${songId}`);
...
export async function fetchAlbumSongs(albumId, limit = 30) {
  try {
    const data = await safeFetch(`${API_BASE}/saavn-search/albums?id=${albumId}`);
...
export async function fetchLyrics(songId) {
  try {
    const data = await safeFetch(`${API_BASE}/api/lyrics/${songId}`);
